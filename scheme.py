...
class LambdaProcedure(Procedure):
    ...
    def make_call_frame(self, args, env):
        ...
        # BEGIN PROBLEM 11
        #print('DEBUG:', 'formals: ', args, self.formals, self.body)
        return self.env.make_child_frame(self.formals, args)
        #return env.make_child_frame(args, do_lambda_form(self.formals, env))
        # END PROBLEM 11
...
def do_and_form(expressions, env):
    ...
    # BEGIN PROBLEM 12
    print('DEBUG:', 'EXPR: ', expressions)
    if expressions == nil:
        return True
    while expressions != nil:
        print('DEBUG:', expressions.first, scheme_falsep(expressions.first))
...
def make_let_frame(bindings, env):
    ...
    val = nil
    while bindings != nil:
        current_bind = bindings.first
        check_form(current_bind, 2, 2)
        formal = Pair(current_bind.first, formal)
        val = Pair(scheme_eval(current_bind.rest.first, env), val)
...
def do_define_macro(expressions, env):
    ...
    if not isinstance(head, Pair):
        raise SchemeError
    check_form(expressions, 2)
    check_form(head, 0)
    check_formals(head)
    macro_proc = MacroProcedure(head.rest, body, env)
    env.define(head.first, macro_proc)
    return head.first
    # END Problem 20
